...
public:
    ...
    std::shared_ptr<std::vector<char> > writeBuffer; ///< Data being written
        ...
        asio::serial_port_base::stop_bits opt_stop)
{
	...
	pimpl->io.post([&](){AsyncSerial::doRead(); });
...
void AsyncSerial::close()
{
    ...
    pimpl->open=false;
	pimpl->io.post([&](){AsyncSerial::doClose(); });
...
void AsyncSerial::write(const char *data, size_t size)
{
    ...
    pimpl->io.post(std::bind(&AsyncSerial::doWrite, this));
}
...
{
	pimpl->port.async_read_some(asio::buffer(pimpl->readBuffer, readBufferSize), [&](const std::error_code error, size_t bytes_transferred){
		AsyncSerial::readEnd(error, bytes_transferred);
	});
}
        ...
        asio::serial_port_base::stop_bits opt_stop)
{
	...
	if (pimpl->fd<0) throw(std::system_error(
		std::error_code(),"Failed to open port"));
    ...
    if(status<0  || !isatty(pimpl->fd))
    {
		...
		throw(std::system_error(
			std::error_code(),"Device is not a tty"));
    }
    ...
    switch(baud_rate)
    {
        ...
        default:
        {
            ::close(pimpl->fd);
			throw(std::system_error(
				std::error_code(),"Unsupported baud rate"));
        }
    }
    ...
    if(status<0)
    {
        ::close(pimpl->fd);
		throw(std::system_error(
			std::error_code(),"Can't set port attributes"));
    }
