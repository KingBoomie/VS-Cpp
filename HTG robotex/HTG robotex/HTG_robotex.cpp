...
void CalcFib (unsigned n) {
	global_stream_lock.lock ();
	std::cout << "[" << std::this_thread::get_id () << "] Calculateing fib(" << n << ")\n";
	global_stream_lock.unlock ();
...
int main () {
	...
	//TODO: cuncurrently bind USB ports to motor objects
	...
	std::vector<std::thread> worker_threads(1);
	for (auto &thread : worker_threads) {
		thread = std::thread (std::bind(&WorkerThread, io_service));
	}
